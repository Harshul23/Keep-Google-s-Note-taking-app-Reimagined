...
const Sidebar = () => {
  ...
  const organizationItems = [
    { path: "/graph", icon: Share2, label: "Knowledge Map", color: "text-blue-400" },
    { path: "/backlinks", icon: GitBranch, label: "Linked Notes", color: "text-green-400" },
        ...
        <div className="flex items-center justify-center w-12 h-12 shrink-0">
          <Icon
            ...
            className={`transition-colors duration-200 ${
              item.color && isActive ? item.color : (isActive ? "text-[#feefc3]" : "text-[#9aa0a6]")
            }`}
          />
        </div>
        <span
  ...
  return (
    ...
    >
      <nav className="pt-2">
        ...
        <div className={`my-2 border-t border-[#5f6368] transition-all duration-300 ${isExpanded ? "mx-4" : "mx-8"}`} />
        
        {isExpanded && (
           <p className="ml-8 mt-2 mb-1 text-[11px] font-bold text-[#9aa0a6] uppercase tracking-widest">Organization</p>
        )}
      ...
      <div className={`absolute bottom-6 left-8 transition-opacity duration-300 ${isExpanded ? "opacity-100" : "opacity-0"}`}>
        <a href="#" className="text-[11px] text-[#9aa0a6] hover:underline whitespace-nowrap">
